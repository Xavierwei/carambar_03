...
package zero.swf.avm2{
	...
	public class ABCClasses{
					...
					if(allMethodV){
						if(method_body_info.method>-1&&method_body_info.method<allMethodV.length){
							...
							if(codeOffset<codeEndOffset){
								...
								var posArr:Array=[startPos];//20131121
								var exceptionArr:Array=new Array();//20131220 发现没有newcatch对应的exception_info，详见 F:\airs\program files2\SWFXML\tests\ExceptionTest
								var exceptionI:int=-1;
								for each(var exception_info:Exception_info in method_body_info.exception_infoV){
									exceptionI++;
									
									var exception:Object=exceptionArr[exceptionI]={
										hasNewCatch:false
									};
									offsetPos=startPos+exception_info.from;
									exception.from=labelByPosArr[offsetPos]||(labelByPosArr[offsetPos]={pos:offsetPos});
									offsetPos=startPos+exception_info.to;
									exception.to=labelByPosArr[offsetPos]||(labelByPosArr[offsetPos]={pos:offsetPos});
									offsetPos=startPos+exception_info.target;
									exception.target=labelByPosArr[offsetPos]||(labelByPosArr[offsetPos]={pos:offsetPos});
									if(exception_info.exc_type>0&&exception_info.exc_type<allMultinameV.length){
										exception.exc_type=allMultinameV[exception_info.exc_type];
									}
									if(exception_info.var_name>0&&exception_info.var_name<allMultinameV.length){
										exception.var_name=allMultinameV[exception_info.var_name];
									}
								...
								while(posArr.length){
									codeOffset=posArr.shift();
									if(codeByPosArr[codeOffset]){
										continue;
									}
									loop:while(codeOffset<codeEndOffset){
										...
										if(AVM2Ops.nameV[code.op]){
											...
											switch(AVM2Ops.valueTypeV[code.op]){
												...
												case "exceptionid":
													if(method_body_info.exception_infoV){
														if(value1>-1&&value1<exceptionArr.length){
															exception=exceptionArr[value1];
															if(exception.hasNewCatch){
																code.info+="重复使用的 exception\n";
															}else{
																exception.hasNewCatch=true;
															}
															code.from=exception.from;
															code.to=exception.to;
															code.target=exception.target;
															code.exc_type=exception.exc_type;
															code.var_name=exception.var_name;
														}else{
															code.op=AVM2Ops.nop;
														}
													}else{
											...
											code.info+="未知 op 0x"+BytesAndStr16._16V[code.op]+"（"+BytesAndStr16.bytes2str16(code.ownData,code.pos,code.dataLen)+"）"+"\n";
								...
								if((labelByPosArr.length-1)>codeEndOffset){
									...
									//2
									//20131213
									//var labelByPosArrCopy:Array=labelByPosArr.slice(codeEndOffset+1);
									//for each(var label:Object in labelByPosArrCopy){
									//	if(label){
									//		label.info="超出代码尾的偏移量："+(labelByPosArr.indexOf(label)-startPos)+"\n";
									//		codeArr[++codeId]=label;
									//	}
									//}
								...
								exceptionI=-1;
								for each(exception in exceptionArr){
									exceptionI++;
									if(exception.hasNewCatch){
									}else{
										codeId=codeArr.indexOf(exception.target);
										code=codeArr[codeId+1];
										if(
											code
											&&
											code.op>-1
											&&
											code.op==AVM2Ops.getlocal0
										){
											code=codeArr[codeId+2];
											if(
												code
												&&
												code.op>-1
												&&
												code.op==AVM2Ops.pushscope
											){
												codeArr.splice(codeId+3,0,
													{
														op:AVM2Ops.newcatch,
														info:"没有 newcatch 对应的 exception，exceptionI="+exceptionI+"，自动生成一个 newcatch\n",
														from:exception.from,
														to:exception.to,
														target:exception.target,
														exc_type:exception.exc_type,
														var_name:exception.var_name
													},
													{op:AVM2Ops.pop}
												);
												continue;
										...
										codeArr.splice(codeId+1,0,{
											op:AVM2Ops.nop,
											info:"没有 newcatch 对应的 exception，exceptionI="+exceptionI+"\n"
										});
									}
								}
		...
		public function toData(_toDataOptions:Object):ByteArray{
			...
			data.position=offset;
			Strings.write(data,stringV,null);
			offset=data.position;
				...
				for each(multiname in allMultinameV){
					...
					switch(multiname.kind){
						...
						case MultinameKinds.GenericName:
							...
							//20131223
							//如果代码其它位置有如：getlex undefined 语句将会影响此结果：
